...
// 编辑语料请求体类型
export interface EditCorpusPayload {
  uuid: string;
  title: string;
  text: string;
  category: string; // 添加：语料分类
...
// 编辑语料响应类型
export interface EditCorpusResponse {
  status: number;
  code?: string; // 成功时可能返回 code
  message?: string; // 失败时返回 message
}
...
export const CreateCorpusAPI = async (
  ...
  try {
    const res = await http.post<CreateCorpusResponse>(
      "/practice-text",
      payload,
    );
...
/**
 * 编辑练习语料
 * @param payload 包含uuid、标题和文本内容
 * @returns
 */
export const EditCorpusAPI = async (
  payload: EditCorpusPayload,
): Promise<EditCorpusResponse> => {
  console.log("调用编辑语料接口, payload:", payload);
  try {
    const res = await http.put<EditCorpusResponse>("/practice-text", payload);
    console.log("编辑语料接口响应:", res.data);
    // 检查返回的数据结构是否符合预期
    if (res.data && typeof res.data.status === "number") {
      return res.data;
    } else {
      console.error("编辑语料接口返回数据格式错误:", res.data);
      return {
        status: 1, // 或者其他错误码
        code: "invalidResponseFormat",
        message: "编辑语料接口返回数据格式错误",
      };
    }
  } catch (error: any) {
    console.error("编辑语料接口错误:", error);
    if (error.response && error.response.data) {
      // 尝试返回后端提供的错误信息
      return error.response.data as EditCorpusResponse;
    }
    // 返回通用错误结构
    return {
      status: 1,
      code: "requestFailed",
      message: error.message || "编辑语料失败",
    };
  }
};
...
export const GetCorpusAPI = async (
  ...
  try {
    const res = await http.get<GetCorpusResponse>("/practice-texts", {
      params,
    });
