...
interface UseMediaRecorderProps {
  onDataAvailable: (data: ArrayBuffer) => void; // 发送 PCM 或 MP3 数据
  onStop?: () => void;
  onError?: (error: Error) => void;
  targetSampleRate?: number; // 添加目标采样率
  mp3BitRate?: number; // 添加 MP3 比特率
  outputFormat?: "pcm" | "mp3"; // 新增：输出格式选项
}
...
export const useMediaRecorder = ({
  ...
  const cleanupAudioResources = useCallback(() => {
    ...
    if (audioContextRef.current && audioContextRef.current.state !== "closed") {
      audioContextRef.current
        .close()
        .catch((e) => console.error("Error closing AudioContext:", e)); // 保留错误日志
    }
  ...
  const stopRecordingInternal = useCallback(
    (sendFinalChunk: boolean = true) => {
      ...
      if (outputFormat === "mp3" && mp3EncoderRef.current && sendFinalChunk) {
    ...
    [cleanupAudioResources, onDataAvailable, onStop, outputFormat],
  );
    ...
    try {
      ...
      try {
        ...
        if (actualSampleRate !== targetSampleRate) {
          console.warn(
            `实际采样率 (${actualSampleRate}Hz) 与目标 (${targetSampleRate}Hz) 不同。MP3 编码将使用目标速率，但如果不进行重采样，输入质量可能会受到影响。`,
          ); // 保留警告
        }
      } catch (e) {
        console.warn(
          `无法使用 ${targetSampleRate}Hz 创建 AudioContext，回退到默认值。错误: ${e}`,
        ); // 保留警告
        // @ts-ignore
        context = new (window.AudioContext || window.webkitAudioContext)();
        actualSampleRate = context.sampleRate;
        console.warn(
          `目标 MP3 采样率 (${targetSampleRate}Hz) 与 context 速率 (${actualSampleRate}Hz) 差异显著。建议进行重采样以保证质量。`,
        ); // 保留警告
      }
      ...
      processorRef.current = context.createScriptProcessor(
      ...
      // 4. 初始化 MP3 编码器 (如果需要)
      if (outputFormat === "mp3") {
        mp3EncoderRef.current = new lamejs.Mp3Encoder(
          1,
          targetSampleRate,
          mp3BitRate,
        );
      ...
      processorRef.current.onaudioprocess = (e: AudioProcessingEvent) => {
        ...
        if (outputFormat === "mp3" && !mp3EncoderRef.current) {
          return;
        }
        ...
        if (outputFormat === "mp3") {
          ...
          // 发送原始 PCM 数据
          onDataAvailable(samples.buffer);
        }
      };
